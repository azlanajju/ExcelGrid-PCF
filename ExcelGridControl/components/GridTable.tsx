          ...
          return (
            <tr key={rIdx} className={row[0] === "Total" ? "total-row" : ""}>
              {row.map((cell, cIdx) => {
                ...
                return <GridCell key={cIdx} value={cell} id={String(rIdx)} row={rIdx} col={cIdx} selected={selected} focused={focused} frozen={frozenCols.includes(cIdx)} fileSetCell={fileSetCells.includes(cIdx)} hasDropdown={hasDropdown} hasUpload={cellHasUpload} isFormula={isFormula} isEditable={isEditable} isTotalRow={isTotalRow} headerStyle={headerStyle} rowIds={rowIds} bodyStyle={bodyStyle} textAlign={alignment.textAlign} onMouseDown={() => onMouseDown(rIdx, cIdx)} onMouseUp={onMouseUp} onCellDropDown={onCellDropDown} onMouseOver={() => onMouseOver(rIdx, cIdx)} onContextMenu={(e) => onContextMenu(e, rIdx, cIdx)} onClick={(e) => onCellClick(rIdx, cIdx, e)} onChange={(value) => onCellChange(rIdx, cIdx, value)} onFocus={() => onCellFocus(rIdx, cIdx)} onBlur={() => validateAndCorrectValue(rIdx, cIdx)} startResize={startResize} formula={isFormula ? getFormula(cIdx) : undefined} frozenLeft={frozenCols.includes(cIdx) ? getFrozenLeft(cIdx) : 0} colRefs={colRefs} cellRefs={cellRefs} tableRef={tableRef} onFileUpdload={onFileUpdload} onFileView={onFileView} headerVal={headerVal} {...props} />;
              })}
            </tr>
          );
        })}
